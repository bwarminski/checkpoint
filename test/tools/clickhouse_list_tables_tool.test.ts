// ABOUTME: Verifies ClickHouse list-tables tool renders plain text table names.
...
test("clickhouse list tables returns a comma-separated table list", async () => {
  ...
  assert.equal(
    await tool.execute({ database: "default" }),
    "query_events, collector_state",
  );
});
